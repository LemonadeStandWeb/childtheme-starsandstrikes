...
function ls_shortcode_promotions()
{
    ...
    if ($ls_shortcode_promotions_query->have_posts()) {
        ...
        while ($ls_shortcode_promotions_query->have_posts()) {
            ...
            $shortcodes .= '[col_inner span__sm="12" bg_color="rgb(255,255,255)" class="special-clickable-card ' . $ls_css_class_map[$ls_promotions_card_color] . '"]';
            $shortcodes .= '[ux_html]';
            $shortcodes .= '<div class="special-box">' . $ls_promotions_card_color . ' Deal</div>';
            $shortcodes .= '<a href="' . $ls_promotions_link . '" class="clickable-card-link"></a>';
            $shortcodes .= '[/ux_html]';

            // Add the promotion image if available
